...
This version keeps the existing calculation and visualization files almost unchanged.
Only `app.py` was minimally updated so that `alpha_deg` and `theta_deg` can be entered from the Streamlit sidebar.

- `alpha_deg`: tire tilt angle used in visualization and `H_pair = 2 * D * sin(alpha) - overlap_height`
- `theta_deg`: interlock angle used in `overlap_height = (L - a*cos(theta)) * tan(theta)`

Deploy with the same file structure:
