...
def _circle_chord_from_sagitta(radius: float, sagitta: float) -> float:
    """Return circle chord length at a given sagitta/depth.

    sagitta=0 means tangent contact, sagitta=radius means the center line.
    The value is clamped to [0, radius] for numerical stability.
    """
...
def _resolve_chord_ratio_fit(
    D: float,
    d: float,
    theta_deg: float,
    chord_fit_ratio: float,
    scan_steps: int = 180,
) -> Dict[str, float | bool]:
    """Select contact depth using only inner-chord / outer-chord comparison.

    No fixed contact chord and no hidden rubber-deformation coefficient are used.
    For each candidate bite depth ``a``:

        c_hole  = chord of receiving tire hole at depth a
        c_outer = chord of inserted tire outer surface at depth a
        ratio   = c_hole / c_outer

    The deepest candidate satisfying ``ratio >= chord_fit_ratio`` is selected.
    ``theta_deg`` only limits the maximum depth attempted: larger theta allows
    trying a deeper bite into the hole.
    """

    if not (0.0 < chord_fit_ratio <= 1.0):
        raise ValueError("끼임 허용 비율은 0보다 크고 1 이하이어야 합니다.")
    ...
    max_depth = max(0.05, R_hole * math.sin(theta))
    max_depth = min(max_depth, R_hole)

    best_fit = None
    best_any = None
    for step in range(1, max(2, scan_steps) + 1):
        a = max_depth * step / max(1, scan_steps)
        c_hole = _circle_chord_from_sagitta(R_hole, a)
        c_outer = _circle_chord_from_sagitta(R_outer, a)
        ratio = c_hole / c_outer if c_outer > 0 else 0.0
        margin = ratio - chord_fit_ratio

        candidate = (a, c_hole, c_outer, ratio, margin)
        if best_any is None or ratio > best_any[3]:
            best_any = candidate
        if ratio >= chord_fit_ratio:
            # Scan is shallow -> deep, so this keeps the deepest accepted bite.
            best_fit = candidate

    if best_fit is None:
        a, c_hole, c_outer, ratio, margin = best_any
        fit = False
    else:
        a, c_hole, c_outer, ratio, margin = best_fit
        fit = True
    ...
    return {
        ...
        "c_hole": c_hole,
        "c_outer": c_outer,
        "chord_fit_ratio_actual": ratio,
        "chord_fit_ratio_threshold": chord_fit_ratio,
        "chord_fit_margin": margin,
        "chord_fit": fit,
    }
...
def calculate_loading(
    ...
    # 기존 c = c_base * r 고정 현 가정을 제거한다.
    # 후보 끼임 깊이 a별로 구멍 내부 현과 타이어 외부 현을 직접 비교한다.
    chord_info = _resolve_chord_ratio_fit(
        D=tire.D,
        d=tire.d,
        theta_deg=theta_deg,
    ...
    R = tire.d / 2.0
    # c에서 역산하지 않고, 현 비교에서 선택된 실제 후보 끼임 깊이 a를 사용한다.
    ...
    N_total = N_face * n_L
    container_fit = bool(chord_info["chord_fit"]) and W1 <= W_c and W2 <= W_c and W3 <= W_c and selected_stack_height <= H_c + (
    ...
    return {
        ...
        "chord_fit_ratio_actual": chord_info["chord_fit_ratio_actual"],
        "chord_fit_ratio_threshold": chord_info["chord_fit_ratio_threshold"],
